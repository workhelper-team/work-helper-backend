...
public interface ExpertQnaService {
    ...
    ExpertQuestionResponse createQuestion(Long caseId, ExpertQuestionRequest request);

    // 일반 사용자가 자신의 사건에 등록된 질문 목록을 조회합니다.
    Page<ExpertQuestionSummaryResponse> getMyQuestions(Long caseId, int page, int size);

    // 일반 사용자가 자신의 사건에 등록된 질문 하나를 상세 조회합니다.
    ExpertQuestionDetailResponse getMyQuestionDetail(Long caseId, Long questionId);

    // 승인된 전문가가 답변할 질문 목록을 조회합니다.
    Page<ExpertQuestionSummaryResponse> getQuestionsForExpert(Long userId, int page, int size);
